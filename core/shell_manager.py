...
class ShellManager:
    ...
    def run(self, input_dir: str, output_csv: str, traineddata_dir: str):
        # 1. Validations
        if not os.path.exists(input_dir):
            self.console.print(f"[bold red]Erro:[/bold red] O diretório de entrada '{input_dir}' não existe.")
            sys.exit(1)
            
        if not os.path.isdir(input_dir):
            self.console.print(f"[bold red]Erro:[/bold red] O caminho de entrada '{input_dir}' não é um diretório.")
            sys.exit(1)

        # 2. Config settings paths
        settings["BASE_PATH"] = os.path.abspath(input_dir)
        settings["OUTPUT_CSV"] = os.path.abspath(output_csv)
        settings["TRAINED_DATA_DIR"] = os.path.abspath(traineddata_dir)
        ...
        with TerminalManager():
            with Live(observer.get_renderable(), console=self.console, refresh_per_second=10) as live:
                ...
                try:
                    ...
                    self.console.print(f"[bold red]🚨 Erro crítico durante o processamento: {e}[/bold red]")
                    sys.exit(1)
        ...
        if successful_pages < total_pages:
            ...
            self.console.print(f"[yellow]O texto extraído das páginas com falha foi salvo em: [bold]{log_path}[/bold][/yellow]\n")
        else:
